...
## 2026-09-10 챕터 이동 버튼.
- 사용자가 지정한 courseMeta 바로 아래에 이전·다음 챕터 버튼을 배치한다.
- 챕터는 Day로 해석한다. 기존 하단 Prev/Next는 카드 이동이므로 새 버튼은 Day를 바로 변경한다.
- 선택 목록의 현재 순서를 재사용해 역순·무작위에서도 일관되게 이동한다. 기존 스타일을 활용하고 새 의존성은 추가하지 않는다.
- 검증 결과. Node 테스트 52개 및 npm run check 통과. Playwright에서 챕터 건너뛰기, 경계, 역순·무작위, 코스 전환, 키보드 Enter·포커스, 모바일 가로 넘침을 확인했다.
- 962×1247 및 390×844 캡처를 확인했다. 다크 모드, Pretendard 실제 로드, 12px 배경 블러, 새 버튼 대비와 포커스가 정상이다.

## 2026-09-10 완료 챕터 복원.
- 마지막 완료한 챕터 자체를 다시 연다. 다음 챕터 자동 이동은 하지 않는다.
- 과정과 Day를 함께 저장하므로 완료 후 다른 과정을 둘러봐도 재접속 시 마지막 완료 위치가 우선한다.
- 기존 localStorage 안전 접근 함수와 리뷰 저장 결과를 재사용한다. 위치 저장 실패는 리뷰 저장 성공과 구분해 표시한다.
- Node 테스트 53개와 npm run check 통과. 브라우저에서 기본동사 Day 002 완료 후 다른 과정으로 이동하고, 새 탭 및 새로고침에서 Day 002 첫 카드 복원을 확인했다.
- 데스크톱·모바일 캡처에서 기존 다크 유리 패널, Pretendard, 대비, 버튼 포커스와 가로 넘침 없음을 확인했다.

## 2026-09-14 사용 허락 제한.
- 사용자는 허락 없는 사용 전부 금지를 선택했다. 저작권자 표기는 저장소 소유 계정 asher8554를 사용한다.
- 라이선스 파일은 없고 README에 MIT License만 있었다. 새 조건은 기존에 적법하게 부여된 허락을 소급 철회하지 않는다.
- 제3자 코드·폰트·학습 자료에는 해당 권리자의 조건이 유지된다. GitHub 공개 저장소 열람·포크 및 법률상 예외는 제한하지 않는다.
- LICENSE와 README의 제한 범위·예외를 대조했고 git diff --check를 통과했다. 실행 코드는 변경하지 않아 런타임 테스트는 생략했다.

## 2026-09-14 작업 폴더 정리와 이중 언어 라이선스.
- 추가 워크트리는 없으며 main 하나만 존재한다. 삭제 범위는 이전 작업에서 생성한 .playwright-cli 스냅샷 6개와 output/playwright 미추적 파일 6개다.
- 추적 중인 glassmorphism-mobile.png는 보존한다. 라이선스의 적용일과 제한 범위, 기존 허락 및 제3자 권리 예외를 변경하지 않고 한글로 병기한다.
- 확인된 절대 경로를 직접 지정해 임시 파일 12개와 빈 .playwright-cli 폴더를 삭제했다. LICENSE 영문·한글의 금지 범위 및 예외를 대조했고 git diff --check를 통과했다. 문서만 변경하여 런타임 테스트는 생략했다.
