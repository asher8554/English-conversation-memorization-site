...
## 2026-07-24 Notion 타임아웃 단계적 대응 계획

**목표.** 동일한 Notion API 30초 타임아웃이 세 번 연속될 때만 재시도와 요청 단계 로그를 사용한다.

**범위.**

- 일반 실행은 기존처럼 한 번만 동기화한다.
- 직전 실패 상태를 Actions 캐시에 보관하고, 성공 또는 다른 종류의 실패에서 초기화한다.
- 직전 두 상태가 타임아웃이면 이번 실행에만 상세 로그와 한 번의 재시도를 적용한다.

**검증 기준.**

- 타임아웃 여부와 진단 모드 결정이 테스트로 검증된다.
- `npm test`와 `npm run check`가 통과한다.
- 수동 실행에서 일반 동기화가 계속 성공한다.
...
## 2026-07-06 기본동사 카드 중복 검증 계획

**목표.** 기본동사 Day 001에서 Next 이후에도 같은 한국어 문장이 다시 보이는 원인을 데이터와 렌더링 양쪽에서 확인하고, 같은 문제가 다른 기본동사 날짜나 영어회화 코스에 있는지 전체 검사합니다.

**가정.** 현재 증상은 Day 001의 첫 `Model Examples` 카드들이 같은 한국어 질문을 반복하는 데이터 문제일 가능성이 높습니다. 다만 실제 브라우저 조작으로 카드 인덱스가 정상 증가하는지도 확인합니다.

**범위.**
- `data.json`의 `basic-verbs`와 `conversation` 전체 카드에서 빈 q/a, 같은 Day 안의 연속 중복 q/a, 섹션별 이상한 반복을 검사합니다.
- `script.js`의 Next, Show Answer, 섹션 라벨 표시 흐름은 증상이 렌더링 문제일 때만 수정합니다.
- live GitHub Pages와 로컬 실행 페이지를 모두 확인합니다.
...
## 2026-09-10 완료 챕터 복원.
Review Complete 저장 성공 시 과정과 Day를 localStorage의 단일 항목에 저장한다. 재접속 시 마지막 완료 챕터의 첫 카드를 복원한다. 잘못되거나 삭제된 챕터는 기존 초기 선택으로 돌아간다.
검증은 저장·복원·실패 처리 테스트와 실제 브라우저 재접속으로 수행한다.

## 2026-09-14 사용 허락 제한.
사용자 소유 자료에 사전 서면 허락 없는 사용·복제·수정·배포를 허용하지 않는 저작권 고지를 LICENSE에 추가하고 README의 MIT 표기를 교체한다. 기존 배포분의 권리와 제3자 권리는 보존한다. 문서 일관성 검사 후 커밋·푸시하고 GitHub 원격 파일을 확인한다.

## 2026-09-14 작업 폴더 정리와 이중 언어 라이선스.
추가 Git 워크트리 유무와 미추적 파일을 확인한 뒤 이번 세션이 만든 검증용 임시 파일만 삭제한다. LICENSE의 영문 조건을 유지하고 대응하는 한글 설명을 추가한다. 문서 검사, 커밋·푸시, 원격 파일 일치와 깨끗한 작업 폴더로 완료를 검증한다.
