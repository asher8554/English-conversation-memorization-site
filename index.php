    ...
    if (empty($env['NOTION_KEY']) || empty($env['NOTION_PAGE_ID'])) {
        ...
        try {
            ...
            if ($count > 0) {
                ...
                $message = "업데이트 실패: 가져온 데이터가 없습니다. (Notion 연결 또는 페이지 내용을 확인해주세요)";
            }
...
if (!file_exists($filename)) {
    // data.json이 없으면 Notion에서 가져오기 시도
    $envPath = __DIR__ . '/.env';
    $imported = false;

    if (file_exists($envPath)) {
        require_once 'NotionImporter.php';
        $env = Utils::loadEnv($envPath);
        ...
        if (!empty($env['NOTION_KEY']) && !empty($env['NOTION_PAGE_ID'])) {
            try {
                ...
                if ($count > 0) {
                    $imported = true;
                    $message = "데이터 파일이 없어 Notion에서 자동으로 가져왔습니다. (총 {$count}일치)";
                }
            } catch (Exception $e) {
                // 실패 시 조용히 넘어가고 빈 파일 생성 (또는 에러 메시지 저장)
                $message = "자동 업데이트 실패: " . $e->getMessage();
            }
        }
    }

    if (!$imported) {
        // 가져오기 실패하거나 설정이 없으면 빈 배열로 초기화
...
list($data, $dayMainSentences) = Utils::processQuizData($jsonData);
?>
<!DOCTYPE html>
<html lang="ko">
